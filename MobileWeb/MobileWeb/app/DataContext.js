...
Conference.dataContext = (function ($) {
    ...
    var querySessions = function (tx) {
      ...
      tx.executeSql('SELECT * FROM sessions WHERE sessions.dayId = "1" ORDER BY sessions.startTime', [], processorFunc, errorDB);
    }

    var queryVenues = function (tx) {
      tx.executeSql('SELECT * FROM venues ORDER BY venues.name', [], processorVen, errorDB);
    }

    var processVenuesList = function (processor) {
      processorVen = processor;
      if (db) {
        db.transaction(queryVenues, errorDB);
      }
    };
    ...
    var pub = {
      ...
      processVenuesList:processVenuesList
    };
